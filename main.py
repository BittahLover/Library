...
class OkWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Бронирование книги")

        layout = QFormLayout()
        self.setLayout(layout)

        self.l_test1 = QLabel("Книга успешно забронирована!")
...
class OrderWindow(QMainWindow):
    def __init__(self, event, event_info, country, city, street, user, name):
        QMainWindow.__init__(self)
        ...
        b_book = QPushButton('Забронировать', self)
        b_book.clicked.connect(lambda: self.booking(event[0][2], event[0][0], user, name))
        layout.addRow(b_book)

        self.l_test1 = QLabel(event[0][1])
        layout.addRow('Название книги', self.l_test1)

        self.l_test1 = QLabel (str(event[0][2]))
        layout.addRow('Количество оставшихся книг', self.l_test1)

        self.l_test1 = QLabel(str(event_info[0][2]))
        layout.addRow('Автор', self.l_test1)
    ...
    @pyqtSlot()
    def booking(self, copies_amount, id, user, name):
        print('Заказ', copies_amount)
        copies_amount -= 1
        ...
        execute_multiple_record([str(copies_amount), str(id)], db_file, sql_update_event_tickets)
        self.close()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, user=None):
        ...
        self.setWindowTitle("BittahLibrary")

        layout = QVBoxLayout()
        self.setLayout(layout)
        print("ROLE", self.role)
        tabs = QTabWidget()
        tabs.addTab (self.show_book_ui(self.list_roles), "Список книг")

        if 'MODERATOR' in self.list_roles:
            tabs.addTab (self.stat_user_ui (), "Статистика пользователей")
            tabs.addTab (self.create_book (), "Создать книгу")
        #tabs.addTab (self.stat_user_ui (), "Статистика пользователей")
    ...
    def show_book_ui(self, list_roles) -> QWidget:
        ...
        self.t_book.setColumnCount(5)
        self.t_book.setHorizontalHeaderLabels(['Название', 'Автор', 'Кол-во копий', 'Дата брони', 'Адрес'])

        match_list = read_table(db_file, sql_select_event_all, None)

        self.t_book.setRowCount(len (match_list))
        ...
        if "USER" in list_roles:
            self.t_book.itemDoubleClicked.connect(self.buy_ticket)
    ...
    def stat_user_ui(self) -> QWidget:
        ...
        self.t_user.setHorizontalHeaderLabels(['Пользователь', 'Заказанные книги', 'Дата регистрации'])
    ...
    def create_book(self) -> QWidget:
        def on_click_add_match():
            ...
            if name.text() == "" or copies_amount.text() == "" or description.text() == "" or author.text() == ""\
            ...
            execute_multiple_record([description.text(), author.text(), date.text()], db_file, sql_insert_event_info)
            event_info_list = read_table(db_file, sql_select_event_info_description, description.text())
            execute_multiple_record([name.text(), copies_amount.text(), event_info_list[0][0], address_list[0][0]],
                                    db_file,
                                    sql_insert_event)
        ...
        city = QLineEdit()
        street = QLineEdit()
        b_si_up = QPushButton('Добавить книгу', self)
        b_si_up.clicked.connect(on_click_add_match)


        layout.addRow('Название', name)
        layout.addRow('Количество копий', copies_amount)
        layout.addRow('Описание', description)
        layout.addRow('Автор', author)
        layout.addRow('Дата брони', date)
        ...
        copies_amount.setValidator(int_validator)
    ...
    def update_log(self):
        ...
        for i in range(len(log_list)):
            if log_list[i][0] != -1:
                ...
                self.t_logg.setItem(i, 0, QTableWidgetItem('Книга (ID: ' + str(log_list[i][1]) + ')'))
            else:
    ...
    def buy_ticket(self):
        ...
        name = self.t_book.selectedItems()[0].text()
        match = read_table(db_file, sql_select_event_name, name)
        ...
        self.window = OrderWindow(match, event_info, country, city, street, self.user, name)
        self.window.show()
        app.exec_()
    ...
    @pyqtSlot()
    def refresh(self, book_list=None):

        if book_list is None:
            book_list = read_table(db_file, sql_select_event_all, None)
        count_row = 0
        for i in book_list:
            if i[2] < 1:
                continue
            if i[2] > 0:
                count_row+=1
        self.t_book.setRowCount(count_row)
        ...
        for book in book_list:
            if book[2] < 1:
                item+=1
            if book[2] > 0:
                event_info = read_table(db_file, sql_select_event_info_id, book [3])
                address_info = read_table(db_file, sql_select_address_id, book [4])
                country = read_table(db_file, sql_select_country_id, address_info [0] [1])
                city = read_table(db_file, sql_select_city_id, address_info [0] [2])
                street = read_table(db_file, sql_select_street_id, address_info [0] [3])

                self.t_book.setItem(index, 0, QTableWidgetItem(book[1]))
                self.t_book.setItem(index, 1, QTableWidgetItem(str(event_info[0] [2])))
                self.t_book.setItem(index, 2, QTableWidgetItem(str(book_list[item][2])))
                self.t_book.setItem(index, 3, QTableWidgetItem(event_info [0] [1]))
                self.t_book.setItem(index, 4, QTableWidgetItem(country [0] [1] + "," + city [0] [1] + "," + street [0] [1]))
                index+=1
                item+=1
